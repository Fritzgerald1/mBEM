...
LenExtend = Nwl*max(WavLen); %半上边界长：从坐标原点距离左右截断位置的长度
L_elem = min(WavLen)/Nelem; % 单元的长度
nElemExtend = round(LenExtend/L_elem); % 半上边界长单元数
R = 10.0;%外圈半径
r = 8.0;%内圈半径
a = 4*pi/8;%弯曲角度
nElemExtend1 = a*R/L_elem;%上弯曲边界的单元个数
nElemExtend2 = a*r/L_elem;%下弯曲边界的单元个数
...
nElemBeneath_B=round(2*H/L_elem); % 弯段和直段连接处的单元个数
if nElemBeneath_B<5
    nElemBeneath_B = 5;
end
