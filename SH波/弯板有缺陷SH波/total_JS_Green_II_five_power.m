function [p,p_m_p]=total_JS_Green_II_five_power(arrRes,Ind,Miu,Rho,H,Omega,n,ElemLen_m_power, ...
...
X1=MidPoint_m_power(:,1);
X2=MidPoint_m_power(:,2)-9;
X3=MidPoint_p_power(:,1);
X4=MidPoint_p_power(:,2)-9;
...
%%能量守恒
R_m=arrRes(Ind(7)+1:Ind(7)+n,1);
R_p=arrRes(Ind(7)+1+n:Ind(7)+2*n,1);

v_m=(-u_Minus.*1i*Omega)*R_m;
v_p=(-u_Plus.*1i*Omega)*R_p;
v_m_p=-u_minus_plus(:,1).*1i*Omega;

t_m=t_Minus*R_m*ElemLen_m_power;
t_p=t_Plus*R_p*ElemLen_p_power;
t_m_p=t_minus_plus(:,1)*ElemLen_m_power;

p_m=(v_m.'*conj(t_m)+conj(v_m.')*t_m)/4;
p_p=(v_p.'*conj(t_p)+conj(v_p.')*t_p)/4;
p=p_m+p_p;
p_m_p=(v_m_p.'*conj(t_m_p)+conj(v_m_p.')*t_m_p)/4;