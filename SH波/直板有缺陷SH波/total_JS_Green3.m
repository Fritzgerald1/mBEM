function [arrRes]...
         =total_JS_Green3(MidPoint,NormalVector,Ind,MarkB,Miu,Rho,H,Omega,n,T,G)
...
x2=MidPoint(:,1)-2;
x1=MidPoint(:,2);
NormalVector_(:,1) = NormalVector(:,2);
NormalVector_(:,2) = -NormalVector(:,1);
...
for loop4=1:n
    ...
    if xx==1
        ...
        t_Plus(:,loop4)=u_Stress_Plus_x1.*NormalVector_(:,1)+u_Stress_Plus_x2.*NormalVector_(:,2);
        u_Stress_Minua_x1=-Miu*1i*kesi*sin(beta*x2).*exp(-1i*kesi*x1);
        u_Stress_Minua_x2=Miu*beta*cos(beta*x2).*exp(-1i*kesi*x1);
        t_Minus(:,loop4)= u_Stress_Minua_x1.*NormalVector_(:,1)+u_Stress_Minua_x2.*NormalVector_(:,2);
    else
...
matA_minus = matA_minus - T(1:nPtS,[1:MarkB(1,1) Ind(2)+1:MarkB(2,1) Ind(4)+1:Ind(5)])*u_Minus([1:MarkB(1,1) Ind(2)+1:MarkB(2,1) Ind(4)+1:Ind(5)],:);
matA_minus = matA_minus + G(1:nPtS,Ind(4)+1:Ind(5))*t_Minus(Ind(4)+1:Ind(5),:);
...
matA_plus = matA_plus - T(1:nPtS,[MarkB(1,2):Ind(2) MarkB(2,2):Ind(4) Ind(5)+1:Ind(6)])*u_Plus([MarkB(1,2):Ind(2) MarkB(2,2):Ind(4) Ind(5)+1:Ind(6)],:);
matA_plus = matA_plus + G(1:nPtS,Ind(5)+1:Ind(6))*t_Plus(Ind(5)+1:Ind(6),:);

matA_minus_ = [1:nPtS];
matA_minus_1 = zeros(nPtS,1);
matA_minus_1 = matA_minus(:,1);
figure(1)
plot(matA_minus_,real(matA_minus_1(1:nPtS,:)),'b')
hold on
figure(2)
plot(matA_minus_,imag(matA_minus_1(1:nPtS,:)),'r--')
hold on
...
matA_minus_plus = matA_minus_plus - T(1:nPtS,[1:MarkB(1,1) Ind(2)+1:MarkB(2,1) Ind(4)+1:Ind(5)])*u_Plus([1:MarkB(1,1) Ind(2)+1:MarkB(2,1) Ind(4)+1:Ind(5)],motai);
matA_minus_plus = matA_minus_plus + G(1:nPtS,Ind(4)+1:Ind(5))*t_Plus(Ind(4)+1:Ind(5),motai);
