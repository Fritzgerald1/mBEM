function [mat_r,mat_dr_D_dn,mat_r_Dxi,mat_dr_Dxi]...
        =GeneGeoInfoMat_MainFrame1(arrX,MidPoint,ElemLen,NormalVector)

nPoint = max(size(MidPoint));

mat_r = cell(nPoint,1);
mat_dr_D_dn = cell(nPoint,1);
mat_r_Dxi = cell(nPoint,1);
mat_dr_Dxi = cell(nPoint,1);
...
for loop1 = 1:nPoint %源点在各边界点中循环
    ...
    mat_dx_dy_temp_ = bsxfun(@minus,arrCoord_S,matCoordCur);
    mat_Cur_r_ = bsxfun(@hypot,mat_dx_dy_temp_(:,1,:),mat_dx_dy_temp_(:,2,:));
    mat_Cur_dr_D_dxi_ = mat_dx_dy_temp_./repmat(mat_Cur_r_,1,2,1);
    
    mat_Cur_dr_D_dn_ = bsxfun(@times,mat_Cur_dr_D_dxi_(:,1,:),NormalVectorT(1,:,:)) + bsxfun(@times,mat_Cur_dr_D_dxi_(:,2,:),NormalVectorT(2,:,:));
    
    mat_Cur_r_ = permute(mat_Cur_r_,[1,3,2]);
    mat_Cur_dr_D_dn_ = permute(mat_Cur_dr_D_dn_,[1,3,2]);
    
    mat_r_Dxi(loop1) = {mat_Cur_r_};
    mat_dr_Dxi(loop1) = {mat_Cur_dr_D_dn_};
