function [arrRes]...
         =total_JS_Green_II_four(MidPoint,NormalVector,midPoint,normalVector,Ind,Miu,Rho,H,Omega,n,MarkB,T,G,t,g)
...
matA_plus_minus = matA_plus_minus - t(1:nPtS,[MarkB(1,2):Ind(3) MarkB(2,2):Ind(6) Ind(7)+1:Ind(8)])*u_plus_minus([MarkB(1,2):Ind(3) MarkB(2,2):Ind(6) Ind(7)+1:Ind(8)],1);
matA_plus_minus = matA_plus_minus + g(1:nPtS,Ind(7)+1:Ind(8))*t_plus_minus(Ind(7)+1:Ind(8),1);
...
matA_minus_ = 1:nPtS;
matA_minus_1 = zeros(nPtS,1);
matA_minus_1 = matA_minus_plus;
figure(1)
plot(matA_minus_,real(matA_minus_1(1:nPtS,1)),'b')
hold on
figure(2)
plot(matA_minus_,imag(matA_minus_1(1:nPtS,1)),'r--')
hold on
%}
%% 求解
