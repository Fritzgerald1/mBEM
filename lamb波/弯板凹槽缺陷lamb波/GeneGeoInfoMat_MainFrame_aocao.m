function [mat_r,mat_dr_D_dn,mat_r_x1,mat_r_x2]...
        =GeneGeoInfoMat_MainFrame_aocao(arrX,MidPoint,ElemLen,NormalVector)

nPoint = max(size(MidPoint));

mat_r = cell(nPoint,1);
mat_dr_D_dn = cell(nPoint,1);
mat_r_x1 = cell(nPoint,1);
mat_r_x2 = cell(nPoint,1);
...
for loop1 = 1:nPoint %源点在各边界点中循环
    ...
    mat_Cur_dr_D_dxi = permute(mat_Cur_dr_D_dxi,[1,3,2]);

    mat_r(loop1) = {mat_Cur_r};
    mat_r_x1(loop1) = {mat_Cur_dr_D_dxi(:,:,1)};
    mat_r_x2(loop1) = {mat_Cur_dr_D_dxi(:,:,2)};
