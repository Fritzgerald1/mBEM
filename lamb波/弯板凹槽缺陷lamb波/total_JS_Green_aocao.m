function [arrRes] = total_JS_Green_aocao(MidPoint,NormalVector,midPoint,normalVector,Ind,MarkB,a,b,cl,ct,w,h1,rr, ...
...
matA_plus(MarkB(2,2):Ind(7),:) = -0.5*u_Plus(MarkB(2,2):Ind(7),:);
matA_plus = matA_plus - T_R(1:nPtS,[MarkB(1,2):Ind(3) MarkB(2,2):Ind(7) Ind(9)+1:Ind(10)])*u_Plus([MarkB(1,2):Ind(3) MarkB(2,2):Ind(7) Ind(9)+1:Ind(10)],:);
matA_plus = matA_plus + G(1:nPtS,Ind(9)+1:Ind(10))*t_Plus(Ind(9)+1:Ind(10),:);

matA = zeros(nPtS,1);
matA(1:MarkB(1,1),:) = -0.5*u_Minus_Plus(1:MarkB(1,1),t);
matA(Ind(3)+1:MarkB(2,1),:) = -0.5*u_Minus_Plus(Ind(3)+1:MarkB(2,1),t);
matA = matA - T_R(1:nPtS,[1:MarkB(1,1) Ind(3)+1:MarkB(2,1) Ind(8)+1:Ind(9)])*u_Minus_Plus([1:MarkB(1,1) Ind(3)+1:MarkB(2,1) Ind(8)+1:Ind(9)],t);
