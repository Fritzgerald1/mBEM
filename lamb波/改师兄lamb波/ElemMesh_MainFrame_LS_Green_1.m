function [MidPoint,ElemLen,NormalVector,arrIndex,MarkB,nElemExtend_h,nElemExtend_R]=ElemMesh_MainFrame_LS_Green_1(H)
...
R=0.05e-3;%%%%缺陷二分之一宽度
h=0.1e-3;%%%%缺陷深度
nElemExtend_TOP = round((LenExtend-R)/L_elem1); % 半上边界长单元数
nElemExtend_BOTTOM = round(LenExtend/L_elem);
nElemExtend_h = round(h/L_elem1);
nElemExtend_R = round(2*R/L_elem1)+1;

nElemBeneath_B=round(2*H/L_elem1);
if nElemBeneath_B<5
    nElemBeneath_B = 5;
end
...
%上边界
x1 = linspace(-LenExtend,-R,nElemExtend_TOP+1); 
...
x2 = linspace(R,LenExtend,nElemExtend_TOP+1); 
...
%下边界
x3 = linspace(-LenExtend,0,nElemExtend_BOTTOM+1); 
...
x4 = linspace(0,LenExtend,nElemExtend_BOTTOM+1); 
...
MarkB(1,1) = arrIndex(1)-10;
MarkB(1,2) = arrIndex(1)+40;
MarkB(2,1) = arrIndex(3)-15;
MarkB(2,2) = arrIndex(3)+15;
