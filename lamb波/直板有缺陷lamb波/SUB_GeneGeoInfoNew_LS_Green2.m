...
NormalVector_M = [NormalVector(1:Ind(2),:);-NormalVector(Ind(2)+1:Ind(4),:);-NormalVector(Ind(4)+1:Ind(5),:);NormalVector(Ind(5)+1:Ind(6),:);-NormalVector(Ind(6)+1:Ind(7),:)];  %法向：上表面向上，下表面向下

Ind_M = [Ind(1:4,:); Ind(5); Ind(6); Ind(7)];

[mat_r_up,mat_dr_D_dn_up,mat_r_x1,mat_r_x2] = GeneGeoInfoMat_MainFrame2(arrX,MidPoint_M,ElemLen_M,NormalVector_M);
