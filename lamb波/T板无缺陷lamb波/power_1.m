function [p_m,p_p1,p_p2,p_m_p]=power_1(a,b,cl,ct,w,h1,num_s,num,t,P_input,k_all,R,ElemLen_m_power,...
...
u_Plus1(2*ii,:)=u22(ii,:);    
t_Plus1(2*ii-1,:)=Txx_tra(ii,:);
t_Plus1(2*ii,:)=Txz_tra(ii,:);

u_Plus2(2*ii-1,:)=u11(ii,:);
u_Plus2(2*ii,:)=u22(ii,:);    
t_Plus2(2*ii-1,:)=Txx_tra(ii,:);
t_Plus2(2*ii,:)=Txz_tra(ii,:);
%%能量守恒
R_m=R(1:num);
R_p1=R(num+1:2*num);
R_p2=R(2*num+1:3*num);
...
v_p1(:,iii)=(-u_Plus1(:,iii)*1i*w)*R_p1(iii);
v_p2(:,iii)=(-u_Plus2(:,iii)*1i*w)*R_p2(iii);
t_m(:,iii)=t_Minus(:,iii)*R_m(iii);
t_p1(:,iii)=t_Plus1(:,iii)*R_p1(iii);
t_p2(:,iii)=t_Plus2(:,iii)*R_p2(iii);
p_m(iii)=real((v_m(:,iii).'*conj(t_m(:,iii)))*ElemLen_m_power/2);
p_p1(iii)=real((v_p1(:,iii).'*conj(t_p1(:,iii)))*ElemLen_p_power/2);
p_p2(iii)=real((v_p2(:,iii).'*conj(t_p2(:,iii)))*ElemLen_p_power/2);
end
